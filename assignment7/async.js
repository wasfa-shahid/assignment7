...
const Id = prompt("Enter Fetch id?");
fetchData(Id)
  .then((users) => {
    console.log(users);
  })
...
function timeout(value) {
  return new Promise((resolve) => {
    setTimeout(() => {
      counter2++;
      resolve({ counter2, value });
    }, 1000);
  });
}

async function incrementCounter1(value) {
  const result = await timeout(value);
  console.log(`x Value ${result.value} Counter: ${result.counter2}`);
}

async function runLoop() {
  for (let i = 1; i <= 10; i++) {
    await incrementCounter1(i)
  }
}
